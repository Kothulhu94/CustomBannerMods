...
namespace Landlord
{
    ...
    {
        private static readonly string LogFilePath = GetLogFilePath("Landlord.log");
        ...
        public static void Log(string message)
        {
            try
            {
                lock (_lock)
                {
                    ...
                    {
                        string gameTime = "N/A";
                        try { gameTime = TaleWorlds.CampaignSystem.CampaignTime.Now.ToString(); } catch {}

                        sw.WriteLine($"[{DateTime.Now:HH:mm:ss.fff}] [Game: {gameTime}] {message}");
                        sw.Flush(); 
                    }
                }
            }

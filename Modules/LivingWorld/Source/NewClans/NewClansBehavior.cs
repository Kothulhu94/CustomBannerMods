...
namespace NewClans
{
    public class NewClansBehavior : CampaignBehaviorBase
    {
        ...
        private void EnforceKingdomColors()
        {
        ...
        private void companion_promote_consequence()
        {
            try
            {
                ...
                var clans = kingdom.Clans.Where(c => c != Clan.PlayerClan && c != factionLeader.Clan && !c.IsEliminated && c.Settlements.Count > 1).ToList();
                ...
                if (settlementToTake != null && sourceClan != null)
                {
                    ...
                    Clan newClan = companion.Clan;
                    _logger.Information("Created new clan: {ClanName}", clanName);

                    // Bug Fix: Ghost Clan Prevention (Prevents GeographicalAdvantageForFaction Crash)
                    if (newClan != null && newClan.Settlements.Count == 0 && settlementToTake != null)
                    {
                        _logger.Warning($"[NewClans Fix] Ghost Clan detected: {newClan.Name} has 0 settlements after creation. Forcing transfer of {settlementToTake.Name}.");
                        try
                        {
                             ChangeOwnerOfSettlementAction.ApplyByDefault(companion, settlementToTake);
                        }
                        catch (Exception ex)
                        {
                             _logger.Error(ex, "[NewClans Fix] Failed to force settlement transfer.");
                        }
                    }
                ...
                else
                {
                    ...
                    InformationManager.DisplayMessage(new InformationMessage(new TaleWorlds.Localization.TextObject("{=lw_nosuitablefieff}No suitable fief found for promotion! Refunding.").ToString()));
                }
            }
            catch (Exception ex)
            {
        ...
        private void OnWeeklyTick()
        {
             ...
             foreach(var clan in Clan.All.ToList())
             {
                 ...
                 if (clan.Tier >= _settings.AiMinTier && clan.Gold > _settings.AiMinGold && clan.Settlements.Count >= _settings.AiMinFiefs)
                 {
        ...
        private void ManageAiPromotion(Clan clan)
        {
            ...
            var candidates = clan.Companions.Where(h => 
                h.IsAlive && 
                h.IsPlayerCompanion == false &&  
                h.IsWanderer && 
                h.Culture == clan.Culture && // STRICT CULTURE MATCH
            ...
            // Fief to give: One of their own Castles, NOT UNDER SIEGE
            Settlement fief = clan.Settlements.FirstOrDefault(s => s.IsCastle && !s.IsUnderSiege);
            if (fief == null) return;
        ...
        private void PerformPromotion(Hero companion, Settlement fief)
        {
             try
             {
                 // Capture old party before state change
                 MobileParty oldParty = companion.PartyBelongedTo;
                ...
                Clan.CreateCompanionToLordClan(companion, fief, new TextObject(clanName), iconId);
                 ...
                 // CRITICAL FIX: Ensure they join the Kingdom 
                 if (newClan != null && companion.Clan.Kingdom == null && fief != null && fief.MapFaction is Kingdom sponsorKingdom)
                 {
                     ChangeKingdomAction.ApplyByJoinToKingdom(newClan, sponsorKingdom);
                     _logger.Information($"Forced {clanName} to join {sponsorKingdom.Name}.");
                 }

                 // Bug Fix: Ghost Clan Prevention
                 if (newClan != null && newClan.Settlements.Count == 0 && fief != null)
                 {
                     _logger.Warning($"[NewClans Fix] Ghost Clan detected (AI): {newClan.Name} has 0 settlements. Forcing transfer of {fief.Name}.");
                     try
                     {
                     ...
                     {
                        _logger.Error(ex, "[NewClans Fix] Failed to force settlement transfer (AI).");
                     }
                 }
                 ...
                 if (oldParty != null)
                 {
                     if (oldParty.MemberRoster.Contains(companion.CharacterObject))
                     {
                         oldParty.MemberRoster.RemoveTroop(companion.CharacterObject, 1);
                     }
                 }

                 if (fief != null)
                 {
                     TeleportHeroAction.ApplyImmediateTeleportToSettlement(companion, fief);
                     _logger.Information($"Teleported {companion.Name} to {fief.Name} to prevent state conflict.");
                 }
             }
             catch (Exception ex)
             {

...
namespace Landlord
{
    public class ViolenceBehavior : CampaignBehaviorBase
    {
        ...
        {
            campaignStarter.AddGameMenuOption("village", "village_landlord_raze", "{=lw_burnhostileplot}Burn Hostile Plot",
                ...
                (MenuCallbackArgs args) =>
                {
                    ...
                    if (hostilePlot != null)
                    {
                        ...
                        InformationManager.DisplayMessage(new InformationMessage(new TaleWorlds.Localization.TextObject($"{{=lw_youburnedhostil}}You burned {hostilePlot.Owner.Name}'s plot!").ToString()));
                    }
                    GameMenu.SwitchToMenu("village");
                }, false, 2, false);


            campaignStarter.AddGameMenu("village_landlord_raze_menu", "{=lw_youapproachthee}You approach the estate. It is currently owned by {OWNER_NAME}.",
                (MenuCallbackArgs args) => 
                {
            ...
            campaignStarter.AddGameMenuOption("village_landlord_raze_menu", "raze_plot", "{=lw_razeplot}Raze Plot",
                (MenuCallbackArgs args) =>
                {
                    args.optionLeaveType = GameMenuOption.LeaveType.Raid;
                    var plot = LandlordManager.Instance.SelectedPlot;
                ...
                (MenuCallbackArgs args) =>
                {
                    ...
                    if (plot != null)
                    {
                         ...
                         if (productionItem != null)
                         {
                             ...
                             InformationManager.DisplayMessage(new InformationMessage(new TaleWorlds.Localization.TextObject($"{{=lw_pillagedlootgol}}Pillaged: {lootGold} gold and 5 {productionItem.Name}").ToString()));
                         }
                         else
                         {
                             InformationManager.DisplayMessage(new InformationMessage(new TaleWorlds.Localization.TextObject($"{{=lw_pillagedlootgol}}Pillaged: {lootGold} gold").ToString()));
                         }
                         ...
                         if (!Hero.MainHero.MapFaction.IsAtWarWith(plot.Owner.MapFaction))
                         {
                             ...
                             InformationManager.DisplayMessage(new InformationMessage(new TaleWorlds.Localization.TextObject("{=lw_youhavecommitte}You have committed an act of war!").ToString()));
                         }
                         else
                         {
                         ...
                         plots.Remove(plot);
                         ModifyHearths(settlement.Village, -GlobalSettings.Instance.HearthDamageOnRaze);
                         InformationManager.DisplayMessage(new InformationMessage(new TaleWorlds.Localization.TextObject($"{{=lw_youburnedplotow}}You burned {plot.Owner.Name}'s plot!").ToString()));
                    }
                    GameMenu.SwitchToMenu("village");
                }, false, -1, false);

            campaignStarter.AddGameMenuOption("village_landlord_raze_menu", "leave_raze", "{=lw_leave}Leave",

    ...
    public class ArmyManagementExecuteDonePatch
    {
        ...
        private static void RecruitWithRenown(ArmyManagementVM vm)
        {
            ...
            if (Clan.PlayerClan.Renown >= totalRenownCost)
            {
                ...
                InformationManager.DisplayMessage(new InformationMessage(new TaleWorlds.Localization.TextObject($"{{=lw_recruitedpartie}}Recruited {partiesInCart.Count} parties for {totalRenownCost} Renown.").ToString(), Colors.Green));
            }
            else
            {
                InformationManager.DisplayMessage(new InformationMessage(new TaleWorlds.Localization.TextObject($"{{=lw_notenoughrenown}}Not enough Renown! Needed: {totalRenownCost}.").ToString(), Colors.Red));
            }
        }
    }

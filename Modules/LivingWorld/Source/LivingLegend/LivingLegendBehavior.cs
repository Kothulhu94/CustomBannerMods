...
namespace LivingLegend
{
    public class LivingLegendBehavior : CampaignBehaviorBase
    {
        ...
        private void OnDailyClanTick(Clan clan)
        {
            ...
            if (clan == Clan.PlayerClan)
            {
                if (clan.Tier >= 6 && clan.Renown > _settings.RenownThreshold)
                {
                    ...
                    InformationManager.DisplayMessage(new InformationMessage(new TaleWorlds.Localization.TextObject($"{{=lw_thelegendsaurai}}The Legend's Aura inspires your people. (-{_settings.DailyRenownCost} Renown)").ToString(), Colors.Green));
                }
            }
            ...
            for (int i = _patrolParties.Count - 1; i >= 0; i--)
            {
                ...
                if (party != null && party.IsActive && CampaignTime.Now >= expiry)
                {
                    ...
                    InformationManager.DisplayMessage(new InformationMessage(new TaleWorlds.Localization.TextObject("{=lw_amilitiapatrolh}A militia patrol has completed its duty and disbanded.").ToString(), Colors.Yellow));
                }
        ...
        private void CallToArmsConsequence()
        {
            ...
            ChangeRelationAction.ApplyPlayerRelation(target, -_settings.CallToArmsRelationPenalty);
        }

        private void AddHeroOfThePeopleMenus(CampaignGameStarter starter)
        {
            starter.AddGameMenuOption("village", "living_legend_rally_militia", $"{{=lw_rallythepeasant}}Rally the Peasantry ({_settings.RallyMilitiaCost} Renown)", 
        ...
        public void SpawnMilitiaPatrol(Settlement village)
        {
             ...
             if (village.GatePosition == CampaignVec2.Zero)
             {
                 Logger?.LogError($"Rally Militia Failed: {village.Name} has invalid/zero GatePosition. Aborting spawn to prevent crash.");
                 return;
             }

             Logger?.LogInformation($"Rally Militia: Spawning patrol at {village.Name} (Pos: {village.GatePosition})");
             ...
             {
                 Logger?.LogError($"Rally Militia Failed: {village.Name} has no valid BasicTroop culture definition.");
                 DestroyPartyAction.Apply(null, militiaParty);
                 return;
             }
             ...
             {
                 Logger?.LogError(ex, $"Critical Failure during Militia Party Initialization for {village.Name}. Destroying party.");
                 if (militiaParty.IsActive) DestroyPartyAction.Apply(null, militiaParty);
             }
        }
        ...
        private void ManageAiRenownActions(Clan clan)
        {
            ...
            if (!_settings.AI_EnableLegendLogic) return;
            if (clan == Clan.PlayerClan) return;
            if (clan.Tier < 6) return;

            if (clan.Renown > _settings.AI_MinRenownForRally)
            {
                foreach (var settlement in clan.Settlements)
                {
                    if (settlement.IsVillage && settlement.Village.VillageState == Village.VillageStates.Normal)
                    {
                        ...
                        if (!hasPatrol)
                        {
                            // Chance per day per village
                            if (MBRandom.RandomFloat < _settings.AI_RallyChance)
                            {
